...
### 首页、动态与液态玻璃
- 重做底栏液态玻璃指示器的动画绑定：指示器位置、宽度和选中态颜色改为跟随页面滑动进度连续插值，首页滑到动态、动态滑回首页时都能逐帧过渡，不再等动画接近结束后突然上色。
- 底栏动画改为可打断过程动画，快速连续切换 tab 或手势滑动中途反向时，会从当前视觉位置继续过渡，避免指示器回弹、跳帧或颜色状态滞后。
- 调整底栏内容采样与可见内容层的同步方式，图标、文字和玻璃指示器在移动过程中使用同一套进度状态，减少液态玻璃折射下图标文字先后变色、选中态错位的问题。
- 提高底栏指示器移动态的主题色混入比例和透明度下限，让浅色背景、深色背景以及动态取色主题下的滑动反馈都更稳定可辨。
