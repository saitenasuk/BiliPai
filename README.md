...
<p align="center">
  <sub>最后更新：2026-03-22 · 文档已同步至 v7.1.0（以 <a href="CHANGELOG.md">CHANGELOG</a> 与源码为准）</sub>
</p>
...
> [!CAUTION]
> 历史记录的“全部删除”当前存在已知问题，暂时不要使用“全部删除”；如需清理，请改用单条删除或谨慎分批处理。
...
> [!TIP]
> 路线图最后同步于 2026-03-22（v7.1.0）。功能以最新 Release、`CHANGELOG.md` 与主分支代码为准。
...
- 📺 **番剧播放器继续向普通视频播放器对齐**：番剧横屏/全屏场景复用更多共享 overlay 能力，整体控制层更统一。
- 👍 **横屏顶部操作补齐为真实可用**：番剧横屏顶部现已支持点赞、投币、分享，不再出现按钮显示但无法点击的情况。
- 🔗 **番剧分享信息更完整**：分享链路改为番剧专用分享链接，标题会自动拼接番剧名与当前剧集名。
- 🔄 **状态反馈更接近普通视频播放器**：点赞状态、投币状态、硬币余额查询与投币结果反馈会跟随真实状态同步刷新。
- ⚠️ **历史记录批量全删仍需回避**：当前已知问题尚未修复，清理历史记录时请改用单条删除或谨慎分批处理。
