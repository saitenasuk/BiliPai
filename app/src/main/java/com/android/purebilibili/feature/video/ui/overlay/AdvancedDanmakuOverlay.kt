...
private fun RenderSingleAdvancedDanmaku(
    ...
    // 转换为像素坐标
    // Bilibili 弹幕坐标通常是 0.0~1.0 或 0~1000 归一化坐标
    // AdvancedDanmakuData 中存储的是 0.0~1.0
    // 高能弹幕居中时需减去文字宽度的一半 (简化起见，这里假设 startX/Y 是中心点)
    ...
    // [视觉优化] 高能弹幕使用更强烈的动画效果
    val scale = if (danmaku.maxCount > 1) {
        // 增长阶段有心跳效果
        if (currentPosition < danmaku.startTimeMs + danmaku.accumulationDurationMs) 1.2f else 1.0f
    } else 1.0f
    ...
    Text(
        ...
        modifier = Modifier
            ...
            // .scale(scale) // 需要 import androidx.compose.ui.draw.scale
    )
}
